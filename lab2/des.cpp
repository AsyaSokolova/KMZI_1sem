...
static const unsigned char expa_perm[48] = { //функция расширения 32 битного вектора до 48 по const таблице Е
	32 , 1 , 2 , 3 , 4 , 5 , 4 , 5 , 6 , 7 , 8 , 9 , 8 , 9 , 10 , 11 ,
	12 , 13 , 12 , 13 , 14 , 15 , 16 , 17 , 16 , 17 , 18 , 19 , 20 , 21 , 20 , 21 ,
	22 , 23 , 24 , 25 , 24 , 25 , 26 , 27 , 28 , 29 , 28 , 29 , 30 , 31 , 32 , 1
} ;

const static unsigned char sbox[8][64]={ //6 битовый блок преврааем в 4 битовый
       	{//S1
...
{
	for(size_t i = 0 ; i < right.size() ; ++i)  //разбиваем на правй и левй блок
...
{//). Результат этой функции побитно суммируется  по модулю «2» (операция ИСКЛЮЧАЮЩЕЕ ИЛИ) с матрицей L0. Полученная матрица используется, как матрица R1 – входная для следующего шага. В качестве входной матрицы L1 для второго шага используется матрица R0.  
	Code code ;//48
	HBlock pcode ;//32
...
const static unsigned char key_table[56] = { // ключ размером 64 бита преобразуем в 56 бит
